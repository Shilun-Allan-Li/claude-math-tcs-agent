...
The two kinds use different templates, since the objects are genuinely different: `distill/individual-template.md` captures a person's **heuristic mind** (first moves, reframings, tool choices); `distill/batch-template.md` captures a domain's **playbook** (canonical problem types, attack stack, route-selection table). Written-style concerns are not in either template — `distill/concise_math_style.md` handles those.
